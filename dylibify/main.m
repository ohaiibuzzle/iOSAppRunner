...
void append_dylib_id_command(FILE *obj_file, off_t header_offset,
                             off_t cmds_end_offset, BOOL is64bit,
                             const char *target) {
  printf("\t\t[*] Appending LC_ID_DYLIB\n");
  ...
  write_bytes(obj_file, cmds_end_offset, cmdsize, dylib_cmd);
  free(dylib_cmd);

  if (is64bit) {
    struct mach_header_64 *mh =
        load_bytes(obj_file, header_offset, sizeof(struct mach_header_64));
    mh->ncmds += 1;
    mh->sizeofcmds += cmdsize;
    write_bytes(obj_file, header_offset, sizeof(struct mach_header_64), mh);
    free(mh);
  } else {
    struct mach_header *mh =
        load_bytes(obj_file, header_offset, sizeof(struct mach_header));
    mh->ncmds += 1;
    mh->sizeofcmds += cmdsize;
    write_bytes(obj_file, header_offset, sizeof(struct mach_header), mh);
    free(mh);
  }
}

void patch_pagezero(FILE *obj_file, off_t offset, struct load_command *cmd,
                    BOOL copied, void *seg, size_t sizeofseg,
                    const char *target) {
  ...
  write_bytes(obj_file, offset, sizeofseg, seg);
}

void patch_dyldinfo(FILE *file, off_t offset,
                    struct dyld_info_command *dyldinfo) {
  if (dyldinfo->rebase_off != 0) {

    //----Some maths takes place in here, we need to iterate over the
    // opcodes----//
    //----Some of them are just 1 byte, some are 2 bytes, some are whole
    // strings----//
    //----We only need the ones referencing to segments, which are 1 byte----//

    for (int i = 0; i < dyldinfo->rebase_size; i++) {
      uint8_t *bytes =
          load_bytes(file, offset + dyldinfo->rebase_off + i, sizeof(uint8_t));

      if ((*bytes & REBASE_OPCODE_MASK) ==
          REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB) {
        printf(
            "\t\t[-] REBASE_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB before = 0x%x\n",
            *bytes);
        *bytes -= 1; // "-1" -> one less segment = previous segment
        write_bytes(file, offset + dyldinfo->rebase_off + i, sizeof(uint8_t),
                    bytes);
        bytes = load_bytes(file, offset + dyldinfo->rebase_off + i,
                           sizeof(uint8_t));
  ...
  if (dyldinfo->bind_off != 0) {
    for (int i = 0; i < dyldinfo->bind_size; i++) {
      uint8_t *bytes =
          load_bytes(file, offset + dyldinfo->bind_off + i, sizeof(uint8_t));

      switch (*bytes & BIND_OPCODE_MASK) {
      case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
        printf(
            "\t\t[-] BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB before = 0x%x\n",
            *bytes);
        if ((*bytes & 0xF) == 2)
          *bytes -= 1;
        write_bytes(file, offset + dyldinfo->bind_off + i, sizeof(uint8_t),
                    bytes);
        bytes =
            load_bytes(file, offset + dyldinfo->bind_off + i, sizeof(uint8_t));
        ...
        while (*bytes != BIND_OPCODE_DO_BIND) {
          i += 1;
          bytes = load_bytes(file, offset + dyldinfo->bind_off + i,
                             sizeof(uint8_t));
        }
        break;
      case BIND_OPCODE_SET_SYMBOL_TRAILING_FLAGS_IMM: // this means a string is
                                                      // coming next
        while (*bytes != 0) { // all strings end with 0 (null byte) and don't
                              // have 0 in their body
          i += 1;
          bytes = load_bytes(file, offset + dyldinfo->bind_off + i,
                             sizeof(uint8_t));
        }
  ...
  if (dyldinfo->lazy_bind_off != 0) {
    for (int i = 0; i < dyldinfo->lazy_bind_size; i++) {
      uint8_t *bytes = load_bytes(file, offset + dyldinfo->lazy_bind_off + i,
                                  sizeof(uint8_t));
      ...
      case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
        ...
        if ((*bytes & 0xF) == 2)
          *bytes -= 1;
        write_bytes(file, offset + dyldinfo->lazy_bind_off + i, sizeof(uint8_t),
                    bytes);
        bytes = load_bytes(file, offset + dyldinfo->lazy_bind_off + i,
                           sizeof(uint8_t));
        printf("\t\t[+] BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB now = 0x%x\n",
               *bytes);
        while (*bytes != BIND_OPCODE_DO_BIND) {
  ...
  if (dyldinfo->weak_bind_off != 0) {
    for (int i = 0; i < dyldinfo->weak_bind_size; i++) {
      uint8_t *bytes = load_bytes(file, offset + dyldinfo->weak_bind_off + i,
                                  sizeof(uint8_t));
      ...
      case BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB:
        ...
        if ((*bytes & 0xF) == 2)
          *bytes -= 1;
        write_bytes(file, offset + dyldinfo->weak_bind_off + i, sizeof(uint8_t),
                    bytes);
        bytes = load_bytes(file, offset + dyldinfo->weak_bind_off + i,
                           sizeof(uint8_t));
        printf("\t\t[+] BIND_OPCODE_SET_SEGMENT_AND_OFFSET_ULEB now = 0x%x\n",
               *bytes);
        while (*bytes != BIND_OPCODE_DO_BIND) {
...
int dylibify(const char *macho, const char *saveto) {
  ...
  //----This variable will hold the binary location as we move on through
  // reading it----//
  size_t offset = 0;
  BOOL copied = false;
  int ncmds = 0;
  struct load_command *cmd = NULL;
  uint32_t *magic = load_bytes(
      file, offset, sizeof(uint32_t)); // at offset 0 we have the magic number
  printf("[i] MAGIC = 0x%x\n", *magic);
  ...
  if (*magic == 0xFEEDFACF) {
    ...
    //----Patch filetype and add MH_NO_REEXPORTED_DYLIB flag (required for
    // linking with it)----//
    ...
    for (int i = 0; i < ncmds; i++) {
      ...
      if (cmd->cmd == LC_SEGMENT_64) {
        ...
        //----Dylibs don't have the PAGEZERO segment, replace it with a
        // LC_ID_DYLIB command----//
        ...
        //----Since we removed one segment we have to to rework opcodes so DATA
        // is not confused with LINKEDIT----//
    ...
    append_dylib_id_command(file, header_offset, offset, true, saveto);
  }
  //----32bit magic number----//
    ...
    struct mach_header *mh =
        load_bytes(file, offset, sizeof(struct mach_header));
    off_t header_offset = offset;
    ...
    append_dylib_id_command(file, header_offset, offset, false, saveto);
  }
  //----More than one architecture----//
    ...
    struct fat_header *fat =
        load_bytes(file, offset, sizeof(struct fat_header));
    struct fat_arch *arch =
        load_bytes(file, arch_offset, sizeof(struct fat_arch));
